...
async function main() {
    console.log('Starting optimized book extraction from Wikipedia categories...');
    ...
    // Optimized list focusing on the most productive categories
    ...
    console.log(`Processing ${optimizedCategories.length} optimized book categories...`);

    for (let i = 0; i < optimizedCategories.length; i++) {
        const category = optimizedCategories[i];
        console.log(`\n[${i + 1}/${optimizedCategories.length}] Processing Category: ${category}`);
    ...
    fs.writeFileSync('optimized-book-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('optimized-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - optimized-book-extraction-results.json (complete results)`);
    console.log(`  - optimized-books-with-gutenberg-links.json (clean library)`);
}
