...
async function main() {
    console.log('Starting focused enhanced book extraction from Wikipedia categories...');
    ...
    // Focused list of the most productive categories
    ...
    console.log(`Processing ${focusedCategories.length} focused book categories...`);

    for (let i = 0; i < focusedCategories.length; i++) {
        const category = focusedCategories[i];
        console.log(`\n[${i + 1}/${focusedCategories.length}] Processing Category: ${category}`);
    ...
    const finalResults = {
        ...
        statistics: {
            totalCategories: focusedCategories.length,
    ...
    fs.writeFileSync('focused-enhanced-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('focused-enhanced-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    
    console.log(`\n=== FOCUSED ENHANCED EXTRACTION COMPLETE ===`);
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - focused-enhanced-extraction-results.json (complete results)`);
    console.log(`  - focused-enhanced-books-with-gutenberg-links.json (clean library)`);
}
