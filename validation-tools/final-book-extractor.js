...
async function main() {
    console.log('Starting final book extraction from Wikipedia categories...');
    ...
    // Final optimized list focusing on the most productive categories
    ...
    console.log(`Processing ${finalCategories.length} final book categories...`);

    for (let i = 0; i < finalCategories.length; i++) {
        const category = finalCategories[i];
        console.log(`\n[${i + 1}/${finalCategories.length}] Processing Category: ${category}`);
    ...
    fs.writeFileSync('final-book-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('final-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - final-book-extraction-results.json (complete results)`);
    console.log(`  - final-books-with-gutenberg-links.json (clean library)`);
}
