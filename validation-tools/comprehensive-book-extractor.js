...
async function main() {
    console.log('Starting comprehensive book extraction from Wikipedia categories...');
    ...
    // Expanded list of book categories based on the successful first run
    const comprehensiveCategories = [
        // Core categories (from first run)
        'Historical novels',
        'Medieval literature',
    ...
    console.log(`Processing ${comprehensiveCategories.length} comprehensive book categories...`);

    for (let i = 0; i < comprehensiveCategories.length; i++) {
        const category = comprehensiveCategories[i];
        console.log(`\n[${i + 1}/${comprehensiveCategories.length}] Processing Category: ${category}`);
    ...
    fs.writeFileSync('comprehensive-book-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('comprehensive-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - comprehensive-book-extraction-results.json (complete results)`);
    console.log(`  - comprehensive-books-with-gutenberg-links.json (clean library)`);
}
