...
async function main() {
    console.log('Starting smart enhanced book extraction from Wikipedia categories...');
    ...
    // Smart focused list of the most productive categories
    ...
    console.log(`Processing ${smartCategories.length} smart book categories...`);

    for (let i = 0; i < smartCategories.length; i++) {
        const category = smartCategories[i];
        console.log(`\n[${i + 1}/${smartCategories.length}] Processing Category: ${category}`);
    ...
    const finalResults = {
        ...
        statistics: {
            totalCategories: smartCategories.length,
    ...
    fs.writeFileSync('smart-enhanced-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('smart-enhanced-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    
    console.log(`\n=== SMART ENHANCED EXTRACTION COMPLETE ===`);
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - smart-enhanced-extraction-results.json (complete results)`);
    console.log(`  - smart-enhanced-books-with-gutenberg-links.json (clean library)`);
}
