...
async function main() {
    console.log('Starting streamlined book extraction from Wikipedia categories...');
    console.log('This will target the most productive book categories');
    console.log('to find more books with Project Gutenberg links.');
    console.log('');

    // Streamlined list focusing on the most productive categories
    ...
    console.log(`Processing ${streamlinedCategories.length} streamlined book categories...`);

    for (let i = 0; i < streamlinedCategories.length; i++) {
        const category = streamlinedCategories[i];
        console.log(`\n[${i + 1}/${streamlinedCategories.length}] Processing Category: ${category}`);
    ...
    const finalResults = {
        ...
        statistics: {
            totalCategories: streamlinedCategories.length,
    ...
    fs.writeFileSync('streamlined-book-extraction-results.json', JSON.stringify(finalResults, null, 2));
    ...
    fs.writeFileSync('streamlined-books-with-gutenberg-links.json', JSON.stringify(library, null, 2));
    
    console.log(`\n=== STREAMLINED EXTRACTION COMPLETE ===`);
    ...
    console.log(`\nResults saved to:`);
    console.log(`  - streamlined-book-extraction-results.json (complete results)`);
    console.log(`  - streamlined-books-with-gutenberg-links.json (clean library)`);
}
