...
function extractGutenbergLinks(content) {
    ...
    // Look for various Gutenberg link patterns
    const patterns = [
...
function extractAuthorFromContent(content, title) {
    ...
    // Look for common patterns like "by Author Name" or "Author Name (born..." etc.
...
async function processPage(page) {
    ...
    try {
        ...
        if (!infoResponse || !infoResponse.query || !infoResponse.query.pages) {
            console.log(`  No info found for ${title}`);
            return null;
        }
        ...
        if (!pageData) {
            console.log(`  No page data for ${title}`);
            return null;
        }
        ...
        if (gutenbergLinks.length === 0) {
            console.log(`  No Gutenberg links found in ${title} (length: ${length})`);
            return null;
        }

        const author = extractAuthorFromContent(content, title);
        ...
        console.log(`  Found ${gutenbergLinks.length} Gutenberg link(s): ${gutenbergLinks.join(', ')} (length: ${length})`);
        if (author) {
...
async function main() {
    console.log('Starting Wikipedia Gutenberg link extraction and ranking...');
    ...
    try {
        ...
        for (let i = 0; i < members.length; i++) {
            ...
            try {
                const result = await processPage(member);
                if (result) {
                    results.push(result);
                }
            } catch (error) {
            ...
            if ((i + 1) % 50 === 0) {
                console.log(`\nSaving progress... (${results.length} successful, ${errors.length} errors)`);
        ...
        console.log(`Total articles processed: ${members.length}`);
...
module.exports = { getAllCategoryMembers, processPage, extractGutenbergLinks };
