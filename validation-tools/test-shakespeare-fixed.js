...
function extractTitleAndAuthor(text, expectedTitle, expectedAuthor) {
  ...
  // Look for multi-line title patterns (for Shakespeare plays)
  if (!actualTitle) {
    for (let i = 0; i < Math.min(lines.length - 1, 20); i++) {
      const line1 = lines[i].trim();
      const line2 = lines[i + 1].trim();
      
      // Check for patterns like "THE THIRD PART OF" followed by "KING HENRY VI"
      if (line1.match(/^THE\s+(FIRST|SECOND|THIRD|FOURTH|FIFTH|SIXTH|SEVENTH|EIGHTH|NINTH|TENTH)\s+PART\s+OF$/i) && 
          line2.match(/^KING\s+\w+/i)) {
        
        // Extract the king name and part number
        const partMatch = line1.match(/THE\s+(\w+)\s+PART\s+OF/i);
        const kingMatch = line2.match(/KING\s+(\w+)/i);
        
        if (partMatch && kingMatch) {
          const partNum = partMatch[1].toLowerCase();
          const kingName = kingMatch[1];
          
          // Convert part names to numbers
          const partNumbers = {
            'first': '1', 'second': '2', 'third': '3', 'fourth': '4', 'fifth': '5',
            'sixth': '6', 'seventh': '7', 'eighth': '8', 'ninth': '9', 'tenth': '10'
          };
          
          const partNumber = partNumbers[partNum] || partNum;
          actualTitle = `King ${kingName}, Part ${partNumber}`;
          console.log(`    Found multi-line title pattern: "${actualTitle}"`);
          break;
        }
      }
      
      // Check for patterns like "THE LIFE AND DEATH OF KING RICHARD THE SECOND"
      if (line1.match(/^THE\s+(LIFE\s+AND\s+DEATH\s+OF\s+)?KING\s+\w+/i)) {
        const kingMatch = line1.match(/KING\s+(\w+)/i);
        if (kingMatch) {
          const kingName = kingMatch[1];
          actualTitle = `King ${kingName}`;
          console.log(`    Found life/death pattern: "${actualTitle}"`);
          break;
        }
      }
      
      // Check for patterns like "THE FAMOUS HISTORY OF THE LIFE OF KING HENRY THE EIGHTH"
      if (line1.match(/^THE\s+FAMOUS\s+HISTORY\s+OF\s+THE\s+LIFE\s+OF\s+KING\s+(\w+)/i)) {
        const kingMatch = line1.match(/KING\s+(\w+)/i);
        if (kingMatch) {
          const kingName = kingMatch[1];
          actualTitle = `King ${kingName}`;
          console.log(`    Found famous history pattern: "${actualTitle}"`);
          break;
        }
      }
    }
  ...
  // Look for single-line title patterns (fallback)
  if (!actualTitle) {
    for (const line of lines.slice(0, 30)) {
      ...
      if (titleMatch && titleMatch[1].length > 5 && titleMatch[1].length < 100) {
        ...
        console.log(`    Found single-line title pattern: "${actualTitle}"`);
        break;
      }
    }
  }
...
// Main function
async function main() {
  console.log('🔍 Shakespeare-Only Validation (FIXED)');
  ...
  // Save results
  fs.writeFileSync('shakespeare-valid-fixed.json', JSON.stringify(validBooks, null, 2));
  fs.writeFileSync('shakespeare-invalid-fixed.json', JSON.stringify(invalidBooks, null, 2));
  ...
  console.log(`\n📄 Valid books saved to: shakespeare-valid-fixed.json`);
  console.log(`📄 Invalid books saved to: shakespeare-invalid-fixed.json`);
}
